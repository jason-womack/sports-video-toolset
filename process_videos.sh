...
normalize_files() {
  ...
  if [[ "$SKIP_NORMALIZATION" != true ]]; then
    ...
    # DJI + VID, case-insensitive
    find "$INPUT_DIR" -type f \( \
        -iname 'DJI_*.MP4' -o -iname 'DJI_*.MOV' -o \
        -iname 'VID_*.MP4' -o -iname 'VID_*.MOV' \
      \) | while read -r f; do
      base=$(basename "$f")
...
process_folder() {
  ...
  # include DJI and VID files
  shopt -s nullglob
  ls DJI_*.MP4 DJI_*.MOV VID_*.MP4 VID_*.MOV 2>/dev/null | sort > input.txt
  shopt -u nullglob
...
EOF
  ...
  # === COMBINE FILES ===
  if [[ -f "$out" ]]; then
    log "SKIP: Combined file $out already exists."
  else
    if [[ "$no_crop" == true ]]; then
      log "Combining files (lossless copy concat) → $out"
      cmd=(ffmpeg -y -f concat -safe 0 -i concat.txt -c copy "$out")
    else
      log "Combining files (reencode) → $out"
      cmd=(ffmpeg -y -f concat -safe 0 -analyzeduration 100000000 -probesize 100000000 \
            -i concat.txt -map 0:v:0 -map 0:a:0 -c:v h264_videotoolbox -b:v 60000000)
  ...
  log "Video Info: $width x $height - $duration sec"
  ...
  if [[ "$CFG_CREATED" == true ]]; then
    echo
    echo "[INFO] Config created for $folder: $cfg"
    read -r -p "Proceed with rendering using current config? [y/N]: " ans
    case "${ans,,}" in
      y|yes) log "Proceeding." ;;
      *) log "Skipping rendering for $folder."; cd - >/dev/null; return ;;
    esac
  fi
  ...
  if [[ "${cfg_preview,,}" == "true" ]]; then
    ...
    cmd=(ffmpeg -y -ss "$start_trim" -i "$out" \
         ...
         -t "$PREVIEW_DURATION" -c:v h264_videotoolbox -b:v 60000000 "$preview_out")
    $DRY_RUN && log "DRY-RUN: ${cmd[*]}" || "${cmd[@]}"
  else
    trim_duration=$(awk "BEGIN {print $duration - $start_trim - $end_trim}")
    log "Final render path (no_crop=$no_crop) → $final_out"

    if [[ "$no_crop" == true ]]; then
      # === ZERO REENCODE TRIM ===
      cmd=(ffmpeg -y -ss "$start_trim" -i "$out" \
           -t "$trim_duration" -c copy -movflags +faststart "$final_out")
    else
      # === FULL ZOOM/CROP PATH ===
      cmd=(ffmpeg -y -ss "$start_trim" -i "$out" \
           -vf "crop=w=$crop_w:h=$crop_h:x=$crop_left_px:y=$crop_top_px,
...
shopt -s nullglob
DJI_FILES=("$INPUT_DIR"/DJI_*.MP4 "$INPUT_DIR"/DJI_*.MOV "$INPUT_DIR"/VID_*.MP4 "$INPUT_DIR"/VID_*.MOV)
shopt -u nullglob
